...
dataset = pd.read_csv('EURUSD.txt',nrows = 1500000)
...
dataset3 = pd.pivot_table(dataset, values=['close', 'open', 'fluct'], index=['year', 'month', 'day','hour'], aggfunc=[np.mean,np.max])

#getting rid of multiindex then renaming the colunms
dataset3.reset_index(inplace=True) 
dataset3.columns=['year', 'month', 'day','hour','close_mean','close_max','open_mean','open_max','fluct_mean','fluct_max']
...
#        print('fact = ' + str(dataset4['close_mean'].values[i]))
        Y_values = dataset4.iloc[i, 11:12+z].values
...
cols = list(dataset3.columns[0:8])+sorted(dataset3.columns[8:])
...
for z in range(1,10):
    ...
    for i in range(len(dataset4)):
        ...
        lTransform.append(Y_values)
    dfA = pd.DataFrame(lTransform)
    dfA.columns = ['v07_'+str(z)+'_transformed'+str(i2) for i2 in range(len(Y_values))]    
    dataset3 = pd.concat([dataset3, dfA], axis=1, join='inner')

print('f.0')
dataset3['date'] = [str(dataset3['year'].values[i])+str(dataset3['month'].values[i])+str(dataset3['day'].values[i])+str(dataset3['hour'].values[i]) for i in range(len(dataset3))]
import datetime as dt
dataset3['date']  = [dt.datetime.strptime(d,'%Y%m%d%H') for d in dataset3['date']]
...
print('f.1')
dummyhour = pd.get_dummies(dataset3['hour'])
dummyhour.columns = ['hour'+str(i) for i in list(sorted(set(dataset3['hour'].values)))]
dataset3 = pd.concat([dataset3, dummyhour], axis=1, join='inner')
...
dataset3 = dataset3.drop(['hour0'],1)
...
dataset3.to_csv('dataset3_hour.csv', sep = ',', index = False)
