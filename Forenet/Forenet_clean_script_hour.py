...
#        ('v03' in i and int(i[-1]) < 3) or
        ('v04' in i and int(i[-1]) < 3) or 
        ('v05'in i and int(i[-1]) < 3) or 
#        ('v06' in i and int(i[-1]) < 3)  or
...
forenet.add(Dense(output_dim = 20, init = 'uniform', activation = 'sigmoid', input_dim = X_train.shape[1]))
forenet.add(Dense(output_dim = 10, init = 'uniform', activation = 'sigmoid', input_dim = X_train.shape[1]))
forenet.add(Dense(output_dim = 5, init = 'uniform', activation = 'sigmoid', input_dim = X_train.shape[1]))
#forenet.add(Dropout(0.25))
forenet.add(Dense(output_dim = 1, init = 'uniform', activation = 'sigmoid'))

forenet.compile(optimizer = 'Adam', loss = 'binary_crossentropy', metrics = ['accuracy'])

# Fitting the ANN to the Training set
forenet.fit(X_train, y_train, batch_size = 100, nb_epoch = 500, verbose = 1)
...
dfresult['AE'] = abs(dfresult['fact']-dfresult['expected'])

print('Mean Absolute error: '+ str(round(np.mean(dfresult['AE'].values),5)))
corr = stats.pearsonr(dfresult['fact'], dfresult['expected'])
print('Pearson corrcoeff: '+ str(round(corr[0],3)))
print('Pearson error: ' + str(round((1-corr[0])*1000,3)))
del corr, y_pred
